...
namespace HaulToBuilding
{
    ...
    public class Dialog_BillConfig_Patches
    {
        ...
        public static void DoDesButton(WidgetRow row, Bill_Production dialog)
        {
            if (!HaulToBuildingMod.Settings.DoDes) return;
            if (row.ButtonIcon(TexStorage.SelectStorage, "HaulToBuilding.Des".Translate()))
            {
                Find.Selector.ClearSelection();
                Find.DesignatorManager.Select(new Designator_Storage(dialog));
            }
        }
